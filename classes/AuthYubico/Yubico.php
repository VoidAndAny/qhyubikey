...
class Auth_Yubico {
    ...
    function parsePasswordOTP($str, $delim = '[:]') {
        if (!preg_match("/^((.*)" . $delim . ")?" .
                        ...
                        "([cbdefghijklnrtuvCBDEFGHIJKLNRTUV]{32}))$/", $str, $matches)) {
            /* Dvorak? */
            if (!preg_match("/^((.*)" . $delim . ")?" .
                            "(([jxe.uidchtnbpygkJXE.UIDCHTNBPYGK]{0,16})" .
                            "([jxe.uidchtnbpygkJXE.UIDCHTNBPYGK]{32}))$/", $str, $matches)) {
                return false;
            } else {
                ...
                if ($info['result'] == CURLE_OK) {
                    ...
                    if (preg_match("/status=([a-zA-Z0-9_]+)/", $str, $out)) {
                        ...
                        if (!preg_match("/otp=" . $params['otp'] . "/", $str) ||
                            ...
                            $checksignature =
                                    base64_encode(hash_hmac('sha1', utf8_encode($check), $this->_key, true));
